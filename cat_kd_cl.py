...
indices = [i for i, (*, label) in enumerate(dataset) if label in classes]
...
self.classifier = nn.Linear(64 * 7 * 7, num_classes)  # [num_classes, 6477]
...
self.classifier = nn.Linear(32 * 7 * 7, num_classes)  # [10, 3277]
...
def **init**(self, noise_dim=100, img_channels=1, feature_maps=64):
super(Generator, self).**init**()
[self.net](http://self.net/) = nn.Sequential(
# 입력: (batch_size, noise_dim, 1, 1)
nn.ConvTranspose2d(noise_dim, feature_maps * 4, kernel_size=7, stride=1, padding=0, bias=False),
nn.BatchNorm2d(feature_maps * 4),
nn.ReLU(True),
# (batch_size, feature_maps4, 7, 7)
nn.ConvTranspose2d(feature_maps * 4, feature_maps * 2, kernel_size=4, stride=2, padding=1, bias=False),
nn.BatchNorm2d(feature_maps * 2),
nn.ReLU(True),
# (batch_size, feature_maps2, 14, 14)
...
def forward(self, x):
return [self.net](http://self.net/)(x)
...
class EWC:
def **init**(self, model, dataset, lambda*=1000):
self.models = []
self.lambdas = []
self.params = []
self.fishers = []
...
images, labels = [images.to](http://images.to/)(device), [labels.to](http://labels.to/)(device)
model.zero_grad()
...
images = [images.to](http://images.to/)(device)
labels = [labels.to](http://labels.to/)(device)
...
student_features = student_model.features[0](https://www.notion.so/images)
teacher_features = teacher_model.features[0](https://www.notion.so/images)
...
student_features = student_model.features[2](https://www.notion.so/images)
teacher_features = teacher_model.features[2](https://www.notion.so/images)
...
# Attention Transfer
student_features_prev = student_model.features[0](https://www.notion.so/images)
prev_teacher_features = prev_teacher_model.features[0](https://www.notion.so/images)
...
*, predicted = torch.max(student_logits_for_task.data, 1)
...
if ewc is None:
ewc = EWC(copy.deepcopy(student_model), task_train_dataset, lambda*=lambda_ewc)
else:
